...
test('avoidance releases connectors that start inside a neighbouring node', () => {
  const source = createNode('source', { x: 0, y: 0 }, { width: 120, height: 80 });
  const target = createNode('target', { x: 300, y: 0 }, { width: 120, height: 80 });
  const blocker = createNode('blocker', { x: 192, y: -72 }, { width: 120, height: 120 });
  const connector = createConnector('elbow', 'right', 'left');

  const path = getConnectorPath(connector, source, target, [source, target, blocker]);
  const rect = {
    left: blocker.position.x,
    right: blocker.position.x + blocker.size.width,
    top: blocker.position.y,
    bottom: blocker.position.y + blocker.size.height
  };

  assert.ok(path.points.length >= 4, 'connector should detour when adjacent nodes overlap');

  const approach = path.points.slice(0, -1);
  const clearance = distanceFromPolylineToRect(approach, rect);
  assert.ok(
    clearance >= CONNECTOR_NODE_AVOIDANCE_CLEARANCE - 1,
    `expected connector to maintain approximately ${CONNECTOR_NODE_AVOIDANCE_CLEARANCE}px clearance but measured ${clearance.toFixed(
      1
    )}px`
  );
  const final = path.points[path.points.length - 1];
  const penultimate = path.points[path.points.length - 2];
  assert.ok(Math.abs(final.x - penultimate.x) < 1e-6, 'final approach should remain vertical into the port');
});
