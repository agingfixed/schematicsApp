...
const intersectsRange = (range: Range, node: Node) => {
  ...
  const endsBeforeStart = range.compareBoundaryPoints(Range.END_TO_START, nodeRange) < 0;
  const startsAfterEnd = range.compareBoundaryPoints(Range.START_TO_END, nodeRange) > 0;
  return !(endsBeforeStart || startsAfterEnd);
};

export const applyLinkFormattingToSelection = (editor: HTMLElement): boolean =>
  withEditorSelection(editor, (_selection, range) => {
    const anchors = Array.from(editor.querySelectorAll('a')).filter((anchor) =>
      intersectsRange(range, anchor)
    );
    ...
    anchors.forEach((anchor) => {
      ...
      const decorationTokens = new Set(
        `${anchor.style.textDecoration} ${anchor.style.textDecorationLine}`
          .split(/\s+/)
          .filter(Boolean)
      );
      decorationTokens.add('underline');
      const decorationValue = Array.from(decorationTokens).join(' ');
      anchor.style.textDecoration = decorationValue;
      anchor.style.textDecorationLine = decorationValue;
    });
