...
export const getConnectorStubLength = (_connector: ConnectorModel): number => MIN_STUB_LENGTH;
...
const normalizeElbowWaypoints = (
  ...
  const tidied = tidyOrthogonalWaypoints(start.point, alignedWaypoints, end.point);
  const base = tidied.length ? tidied : buildDefaultWaypoints(connector, start, end);
